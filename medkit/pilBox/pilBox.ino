...
int box[8][4] =  {0, 0, 0, 0,  // box[i][0]: cột trạng thái đóng-0 mở-1
                  0, 0, 0, 0,  // box[i][1]: cột số lượng sáng  {
...
void caiDatSoLuong_sang(int soNganThuoc){
  if (digitalRead(button1) && box[soNganThuoc][1] > 0){
    while(digitalRead(button1)){
    }
//    changeTru(&box[soNganThuoc][1]);
    if(box[soNganThuoc][1] > 0){
      changeTru(&box[soNganThuoc][1]);
    }
  }
  else if (digitalRead(button2) && box[soNganThuoc][1] <= 16){
    while(digitalRead(button2)){
    }
//    changeCong(&box[soNganThuoc][1]);
    if(box[soNganThuoc][1] <16){
      changeCong(&box[soNganThuoc][1]);
    }
  }
...
void caiDatSoLuong_trua(int soNganThuoc){
  if (digitalRead(button1) && box[soNganThuoc][2] > 0){
    while(digitalRead(button1)){
    }
    //soLuongx2_trua[soNganThuoc]--;
//    changeTru(&box[soNganThuoc][2]);
    if(box[soNganThuoc][2] > 0){
      changeTru(&box[soNganThuoc][2]);
    }
  }
  else if (digitalRead(button2) && box[soNganThuoc][2] < 18){
    while(digitalRead(button2)){
    }
//    changeCong(&box[soNganThuoc][2]);
    if(box[soNganThuoc][2] <16){
      changeCong(&box[soNganThuoc][2]);
    }
  }
  //soLuong_trua[soNganThuoc] = soLuongx2_trua[soNganThuoc]*10/2;
//  tinhSoLuong(&box[soNganThuoc][2], &soLuongx2_trua[soNganThuoc]);
Serial.println(box[soNganThuoc][2]);
...
void caiDatSoLuong_toi(int soNganThuoc){
  if (digitalRead(button1) && box[soNganThuoc][3] > 0){
     while(digitalRead(button1)){
     }
//     changeTru(&box[soNganThuoc][3]);
  ...
  else if (digitalRead(button2) && box[soNganThuoc][3] < 18){
          while(digitalRead(button2)){
          }
//          changeCong(&box[soNganThuoc][3]);
          if(box[soNganThuoc][3] <19){
            changeCong(&box[soNganThuoc][3]);
          }
       }
  //soLuong_toi[soNganThuoc] = soLuongx2_toi[soNganThuoc]*10/2;
//  tinhSoLuong(&box[soNganThuoc][3], &soLuongx2_toi[soNganThuoc]);
  Serial.println(box[soNganThuoc][3]);
...
//      if(box[i][tg] > 0 && daLayThuoc[i] == 0){
//        Serial.println("ngan chua lay thuoc");
//          Serial.println(i);
//        batLed(i);
//      }
//      else tatLed(i);
//    }    
    int openedCase = nhanDienMoNap();
    if(openedCase != 0){  //nếu mở nắp 1 trong các ngăn cần lấy thuốc
      turnOffAllLed();
      batLed(openedCase);  // sáng đèn ngăn đó
      hienThiHaiLed(box[openedCase][tg]);      // loa và hiển thi số lượng đến khi đóng nắp
